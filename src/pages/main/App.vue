...
function logout() {
  ...
  //showFullNav.value = !showFullNav.value
}
...
<template>
  <div class="flex h-screen">
    ...
    <div
      :class="'lg:w-17% h-full lg:p-4 lg:pe-0 box-border hidden w-0 lg:block'"
      v-if="route.path !== '/auth'">

      <div class="bg-#F2F2F2 rounded-2xl w-full h-full">
        <div class="flex items-center p4">
          <img src="/src/assets/avatar.jpeg" class="w-0 lg:w-2.5rem aspect-ratio-1 rounded-xl">
        ...
        <div class="">
          ...
          <NavItem name="收件箱" :number="emailStore.unreadCount" :is-selected="currentPath === 'inbox'" destinationPath="/inbox"
            :show-full="showFullNav">
    ...
    <router-view v-slot="{ Component }"
      :class="(route.path !== '/auth' ? 'lg:w-83% w-full' : 'w-full')">
